...
const Charts = () => {
    ...
    useEffect(() => {
        if (loadChart) {
            axios.get(`http://localhost:8080/getBarchart?month=${month}`).then((res) => {
            ...
            axios.get(`http://localhost:8080/getPiechart?month=${month}`).then((res) => {
                console.log(res.data)
    ...
    return (
        <div className='Charts-comp'>
            <h1 className='text-primary'>Graph Visualization</h1>

            <div className="buttons-div">
                <button onClick={loadBarChart} className='chart-btn'>Column chart</button>
                <button onClick={loadPieChart} className='chart-btn'>Sector graph</button>
            </div>
